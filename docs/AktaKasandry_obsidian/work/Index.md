...
- 2026-05-20 — [[work/2026-05-20-public-snapshot-and-osm-map|PUBLIC snapshot + OSM map]] — content source is now a generator over `G:\…\PUBLIC` (`npm run build-content`). Boston map switched from SVG placeholder to OpenStreetMap tiles inside the existing article (no dedicated `/map` route any more).
